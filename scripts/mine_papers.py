...
"""
from arxiv_miner import MiningProcess
...
@click.option(
    "--detex_path",
    default=DEFAULT_DETEX_PATH,
    help="Path To Detex Binary For Latex Processing",
)
@click.option(
    "--mining_interval",
    default=DEFAULT_MINING_INTERVAL,
    help="Interval in Seconds To Wait If a Paper Was Not Mined",
)
@click.option(
    "--mining_limit",
    default=DEFAULT_MINING_LIMIT,
    help="Maximum Number of Papers To Mine",
)
@click.option(
    "--empty_wait_time",
    default=DEFAULT_EMPTY_WAIT_TIME,
    help="Time To Wait if No Unmined Records Were Returned",
)
@click.option(
    "--sleep_interval_count",
    default=DEFAULT_SLEEP_INTERVAL_COUNT,
    help="The Process Will Sleep for `empty_wait_time` after `sleep_interval_count` records",
)
@click.pass_context
def start_miner(
    ctx,  # click context object: populated from db_cli
    num_procs,
    mining_data_path,
    forever=False,
