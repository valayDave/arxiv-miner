...
parser = argparse.ArgumentParser(
    description=f"{DEFAULT_APP_NAME} : Quick streamlist dashboard to search over mined documents"
)
...
class DataView:
    def __init__(self, database: KeywordsTextSearch):
        ...
        self.view_opt = st.sidebar.selectbox(
            "Product Features", ["Search", "Analytics", "Bookmarks"]
        )
        ...
        self.start_date = st.sidebar.date_input(
            "From Date ?", (datetime.datetime.now() - datetime.timedelta(days=2000))
        )
        self.end_date = st.sidebar.date_input(
            "To Date ?", datetime.datetime.now(), max_value=datetime.datetime.now()
        )
        self.sort_key = st.sidebar.selectbox(
            "Sort Your Results By", sort_keys, format_func=lambda x: x["name"]
        )["value"]

        topics = arxiv_miner.get_cs_topics()
        self.selected_topics = st.sidebar.multiselect(
            "Select CS Topics To Look For :",
            topics,
            None,
            lambda x: arxiv_miner.COMPUTER_SCIENCE_TOPICS[x],
        )
        self.page_number = st.sidebar.number_input(
            "Page Number",
            min_value=1,
            value=1,
            step=1,
        )
        self.page_size = st.sidebar.number_input(
            "Page Size",
            min_value=0,
            value=10,
            step=10,
        ...
        self.search_only_survey_papers = st.sidebar.checkbox(
            "Only Search In Survey Papers"
        )
        ...
        if self.search_only_survey_papers:
            if len(self.search_text) > 0:
                ...
                self.db_search_text = (
                    '(identity.title:"*survey*" OR identity.title:"*a review*")'
                )
        else:
            self.db_search_text = self.search_text
        if len(self.search_text) > 0:
            self.text_filter_sections = st.sidebar.multiselect(
                "Do you Want to Look Specifically in some section of the a Paper's Research ?",
                list(FIELD_MAPPING.keys()),
                None,
                lambda x: FIELD_MAPPING[x],
            )
        else:
    ...
    def _show_bookmark_list(self):
        ...
        for key in self.bookmarker.hash_map:
            self.print_block(
                self.bookmarker.hash_map[key]["response"], allow_bookmarking=False
            )
        ...
        fig2 = go.Figure(
            ...
            layout=go.Layout(
                ...
                yaxis=go.layout.YAxis(
                    title="Articles Per %s"
                    % self.choose_time_buckets(str(self.start_date), str(self.end_date))
                ),
            ),
        )
    ...
    def _run_category_distribution_chart(self):
        ...
        plot_x = [
            x if x not in COMPUTER_SCIENCE_TOPICS else COMPUTER_SCIENCE_TOPICS[x]
            for x in plot_x
        ]
    ...
    def _run_date_distribution_chart(self):
        agg_resp = self.db.text_aggregation(
            DateAggregation(
                ...
                page_number=self.page_number,
                page_size=self.page_size,
                calendar_interval=self.choose_time_buckets(
    ...
    def _run_search(self):
        ...
        st.markdown(
            DataView.badge_it(
                "Found %d Articles" % search_resp[0].num_results,
        ...
        for resp in search_resp:
            ...
            if resp.identity.identity in self.bookmarker.hash_map:
                bookmarked = True
            bookmark_button_res, block_identity = self.print_block(
                resp,
                bookmarked=bookmarked,
                survey_paper_filter_enabled=self.search_only_survey_papers,
            )
            if bookmark_button_res:
                self.bookmarker.hash_map[block_identity] = {
                    "response": resp,
                    "searchterms": self.db_search_text,
                }
            else:
                if resp.identity.identity in self.bookmarker.hash_map:
    ...
    def badge_it(tag, badge_structure="badge-pill", badge_type="badge-info"):
        return (
            '<span class="badge {badge_structure} {badge_type}">'.format(
                badge_type=badge_type, badge_structure=badge_structure
            )
            + tag
            + "</span>"
        )
    ...
    def print_block(
        ...
        survey_paper_filter_enabled=False,
    ):
        # TODO : Add The following :
        # TODO :Show context Highlight.
        human_readable_date = dateparser.parse(block.identity.published).strftime(
            "%d, %b %Y"
        )
        cats = [
            arxiv_miner.COMPUTER_SCIENCE_TOPICS[cat]
            if cat in arxiv_miner.COMPUTER_SCIENCE_TOPICS
            else cat
            for cat in block.identity.categories
        ]
        cats = [DataView.badge_it(cat) for cat in cats]
        cats = cats[:3]
        cats_html = "&nbsp;".join(cats)
        date_html = (
            '<span class="badge badge-pill" style="text-align:left">'
            + human_readable_date
            + "</span>"
        )
        ...
        if len(block.result_locations) > 0:
            if survey_paper_filter_enabled:
                if len(block.result_locations) > 1:
                    findings_html = (
                        "<i>Results Found in</i>&nbsp;&nbsp;"
                        + "&nbsp;".join(
                            [
                                DataView.badge_it(cat, badge_type="badge-primary")
                                for cat in block.result_locations
                            ]
                        )
                    )
                else:
                ...
                findings_html = "<i>Results Found in</i>&nbsp;&nbsp;" + "&nbsp;".join(
                    [
                        DataView.badge_it(cat, badge_type="badge-primary")
                        for cat in block.result_locations
                    ]
        ...
        for key in block.highlight_dict.keys():
            ...
            if survey_paper_filter_enabled:
                if "survey" in str.lower(" ".join(block.highlight_dict[key])):
                    continue
            head_str = "<p><b><i>{highlight_heading}<b><i></p>".format(
                highlight_heading=key
            )
            list_str = (
                "<ul>"
                + "".join(
                    [
                        ...
                        + val.replace("#", "").replace("\n", "").strip()
                        + "</li>"
                        for val in block.highlight_dict[key]
                    ]
                )
                + "</ul>"
            )
        ...
        if allow_bookmarking:
            bookmark_button_res = st.checkbox(
                "Bookmark %s" % block.identity.identity, value=bookmarked
            )
        if highlight_print_str == "":
            ...
            """.format(
                abstract=block.identity.abstract,
                highlight_print_str=highlight_print_str,
            )
        ...
        )  # --> Returns button value and identity.


@st.cache(
    hash_funcs={KeywordsTextSearch: id, TextSearchFilter: hash},
    persist=True,
    allow_output_mutation=True,
)
def get_db_data(db_conn, tsf):
...
def text_search_dashboard(
    use_defaults, config_path, host, port, app_name=DEFAULT_APP_NAME
):
