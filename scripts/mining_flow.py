...
class MiningFlow(FlowSpec, DatabaseParameters):
    ...
    detex_binary = IncludeFile(
        "detex-binary",
        default="./detex",
        is_text=False,
        help="Path To Detex Binary For Latex Processing",
    )
    mining_limit = Parameter(
        "mining-limit",
    ...
    def get_miner(self, db_conn, detex_file_path, mining_data_path):
        return MiningEngine(db_conn, mining_data_path, detex_file_path)
    ...
    def end(self):
        print(
            "Was able to complete mining %d / %d"
            % (len(self.success_ids), int(self.mining_limit))
        )
