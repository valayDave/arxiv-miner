...
@click.option(
    "--host",
    default=Config.get_db_defaults()["host"],
    help="ArxivElasticsearchDatabase Host",
)
@click.option(
    "--port",
    default=Config.get_db_defaults()["port"],
    help="ArxivElasticsearchDatabase Port",
)
@click.option(
    "--print_every", default=100, help="Print Message After Saving X objects to FS"
)
@click.option(
    "--limit",
    default=None,
    type=int,
    help="Cap the Limit Of Records to Mine and Send to DB.",
)
@click.option(
    "--buffer-size",
    default=200,
    type=int,
    help="The Number Items to Keep in Batch For Ontology Processing.",
)
@click.option(
    "--workers", default=1, type=int, help="Number of Workers For Ontology Processing."
)
def create_backup(
    host="localhost", port=9200, print_every=100, limit=None, buffer_size=200, workers=1
):
    click.secho(
        "Connecting To Elasticsearch on Host %s And Port %s" % (host, port), fg="green"
    )
    ...
    migrate_db = KeywordsTextSearch(
        Config.elasticsearch_index + "_with_ontology", host=host, port=port
    )
    num_stored = 0
    ...
    for rec in database.parsed_research_stream():
        ...
        migrate_db.set_many_authors(
            [
                Author(name=xp)
                for xp in set([a for x in srp for a in x.identity.authors])
            ]
        )
