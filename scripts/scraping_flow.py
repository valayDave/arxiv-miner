...
DEFAULT_START_DATE = (datetime.datetime.now() - datetime.timedelta(days=1)).strftime(
...
class ScrapingFlow(FlowSpec, DatabaseParameters):
    ...
    end_date = Parameter(
        "end_date", default=DEFAULT_END_DATE, help="End Date in Y-m-d Format"
    )
    timeout_per_date = Parameter(
        "timeout_per_date",
        default=DEFAULT_TIMEOUT_PER_DAILY_SCRAPE,
        help="Time To Wait Before Next Scraping",
    )
    selected_topic = Parameter(
        "arxiv-topic",
