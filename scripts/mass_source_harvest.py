...
@click.option(
    "--max-chunks", default=1, help="Maximum Chunks To Harvest from S3 Bucket"
)
@click.option(
    "--download-rootpath",
    default=DEFAULT_DOWNLOAD_PATH,
    help="Root Path to which Source From Latex Will be Downloaded",
)
@click.option(
    "--sample",
    default=None,
    help="Extract Sampled Data. It will not delete file from remote. Will Do a Dry Run of the code. Sample Value <100 ",
)
def harvest(
    max_chunks=1, download_rootpath=DEFAULT_DOWNLOAD_PATH, type=int, sample=None
):
    for i in range(max_chunks):
        ...
        harvest_engine = SourceHarvestingEngine(
            id_list, download_rootpath, error_sleep_time=5
        )
        download_path_tuples = (
            harvest_engine.harvest()
        )  # returns [(arxiv_id,download_path)]
