...
def create_backup(
    ...
    database = ArxivElasticSeachDatabaseClient(
        Config.elasticsearch_index, host=host, port=port
    )
    ...
    for rec in database.record_stream():
        ...
        save_json_to_file(
            arxiv_object, os.path.join(raw_json_save_path, rec_id + ".json")
        )
        num_stored += 1
    ...
    for rec in database.parsed_research_stream():
        ...
        rec_id = rec.identity.identity
        save_json_to_file(
            arxiv_object, os.path.join(parsed_json_save_path, rec_id + ".json")
        )
        num_stored += 1
        if num_stored % print_every == 0:
    ...
    save_tar_file = tarfile.open(tar_file_path, "w:gz")
