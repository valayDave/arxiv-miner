...
DEFAULT_END_DATE = (datetime.datetime.now() - datetime.timedelta(days=1)).strftime(
    ScrapingEngine.date_format
)
...
def common_run_options(func):
    @click.option(
        "--selected_class",
        default=DEFAULT_SELECTED_CLASS,
        help="Topic Of ArXiv Papers to Scrape",
    )
    @click.option(
        "--thread-mode", is_flag=True, help="Run In Thread Mode Instead of Process Mode"
    )
    @wraps(func)
...
@common_run_options
@click.option(
    "--start_date", default=DEFAULT_START_DATE, help="Start Date in Y-m-d Format"
)
@click.option("--end_date", default=DEFAULT_END_DATE, help="End Date in Y-m-d Format")
@click.option(
    "--timeout_per_scrape",
    default=DEFAULT_TIMEOUT_PER_DATE_RANGE_SCRAPE,
    help="Time To Wait Before Next Scraping",
)
@click.pass_context
def date_range(
    ctx,  # click context
...
@click.option(
    "--timeout_per_scrape",
    default=DEFAUL_TIMEOUT_PER_DAILY_SCRAPE,
    help="Time To Wait Before Next Scraping",
)
@click.pass_context
def daily_harvest(
