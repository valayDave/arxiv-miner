...
class ArxivPaper(ArxivRecord):
    ...
    def tex_files(self):
        file_names = list(
            glob.glob(os.path.join(self.latex_root_path, "**/*.tex"), recursive=True)
        )
        return file_names
    ...
    def _load_metadata_from_fs(self):
        ...
        if processing_meta_dict["latex_processing_meta"]:
            self.latex_parsing_result = ArxivLatexParsingResult(
                **processing_meta_dict["latex_processing_meta"]
            )
        if processing_meta_dict["paper_processing_meta"]:
            self.paper_processing_meta = ArxivPaperProcessingMeta(
                **processing_meta_dict["paper_processing_meta"]
            )
    ...
    def _save_metadata_to_fs(self):
        ...
        processing_result_dict = {
            "latex_processing_meta": None
            if self.latex_parsing_result is None
            else self.latex_parsing_result.to_json(),
            "paper_processing_meta": None
            if self.paper_processing_meta is None
            else self.paper_processing_meta.to_json(),
        }
    ...
    def _save_parsed_document_to_fs(self):
        ...
        if self.latex_parsed_document is not None:
            save_json_to_file(
                self.latex_parsed_document.to_json(), self.tex_processing_file_path
            )
    ...
    def __str__(self):
        ...
        if self.latex_parsed_document is not None:
            ...
            {parsing_outline}
            """.format(
                parsing_outline=parsing_outline
            )
            format_str = format_str + "\n" + latex_parsing_str
    ...
    def _extract_info_from_latex(self):
        ...
        self.paper_processing_meta.pdf_only = (
            True if len(self.tex_files) == 0 else False
        )

        latex_processor = ArxivLatexParser(detex_path=self.detex_path)
        paper_procesesing_results, arxiv_parsed_doc = latex_processor(
            self
        )  # Latex_processor on paper.
        self.paper_processing_meta.mined = (
            True
            if not paper_procesesing_results.parsing_error
            and not self.paper_processing_meta.pdf_only
            else False
        )
    ...
    def _build_paper(self, save_data=True, store_latex=False):
        ...
        try:
            ...
            if not dir_exists(self.paper_root_path):
                os.makedirs(self.paper_root_path)
            # $ Download the paper.
            downloaded_data = arxiv.download(
                paper,
                dirpath=self.paper_root_path,
                slugify=lambda paper: paper.get("id").split("/")[-1],
    ...
    def from_fs(cls, paper_id, root_papers_path, detex_path=None):
        paper = cls(
            paper_id, root_papers_path, build_paper=False, detex_path=detex_path
        )
...
class SingleDocumentLatexParser(LatexInformationParser):
    ...
    def section_extraction(self, tex_file_path) -> List[Section]:
        ...
        if len(tex_node.branches) > self.max_section_limit:
            raise MaxSectionSizeException(
                len(tex_node.branches), self.max_section_limit
            )
    ...
    @staticmethod
    def split_and_find_section(
        curr_text, curr_sec_name, prev_section, split_upto=0.2, split_bins=10
    ):
        ...
        :returns curr_text,Find_status : After removal of redundant section post splitting.
                                         ...
                                         `prev_section` object
        """
        current_text_split = split_match(
            curr_sec_name, curr_text, split_upto=split_upto, split_bins=split_bins
        )
    ...
    def collate_sections(
        self, paper_text, section_list: List[Section], split_upto=0.2, split_bins=10
    ):
        """collate_sections
    ...
    def from_arxiv_paper(
        self, paper: ArxivPaper, lowest_section_match_percent=0.2, number_to_tries=10
    ):
        """from_arxiv_paper
        ...
        sections, some_section_not_found = self.collate_sections(
            tex_in_text,
            sections,
            split_upto=lowest_section_match_percent,
            split_bins=number_to_tries,
        )
...
class ArxivLatexParser:
    ...
    def __init__(self, max_section_limit=20, detex_path=None):
        self.single_doc_parser = SingleDocumentLatexParser(
            max_section_limit=max_section_limit, detex_path=detex_path
        )
        self.multi_doc_parser = MultiDocumentLatexParser(
            max_section_limit=max_section_limit, detex_path=detex_path
        )
    ...
    def __call__(
        ...
        try:
            (
                collected_sections,
                some_sections_failed,
                file_results,
            ) = selected_parser.from_arxiv_paper(
                paper,
            ...
            parsing_result.file_results = [
                {"name": k, "status": file_results[k]} for k in file_results
            ]
        except Exception as e:
    ...
    def _build_document_from_paper(
        self, paper: ArxivPaper, result: ArxivLatexParsingResult
    ):
        if result.section_list is None:
            return None

        sectionised_data = ArxivDocument(
            name=self.parsing_result_name, **paper.identity_meta
        )
...
class ResearchPaperFactory:
    ...
    def from_arxiv_record(paper: ArxivRecord):
        ...
        if paper.latex_parsed_document is None:
            return ResearchPaperSematicParser([]).to_research_paper()

        found_subsections = paper.latex_parsed_document.subsections
        if len(found_subsections) == 0:
            return ResearchPaperSematicParser([]).to_research_paper()

        research_doc = ResearchPaperSematicParser(found_subsections).to_research_paper()
