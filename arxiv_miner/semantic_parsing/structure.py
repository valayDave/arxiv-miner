...
class Section:
    ...
    def to_markdown(self, tab_counter=0):
        ...
        hierarchy = (
            [heading_val + SPACE + self.name + "(" + str(len(self.text)) + ")"]
    ...
    def to_quoted_tags(self):
        ...
        # heading_val = HEADING*tab_counter if tab_counter <=3 else HEADING*3
        hierarchy = (
            [QUOTE_SECTION + self.name + UNQUOTE_SECTION]
            + [QUOTE_CONTENT + self._clean_text(self.text) + UNQUOTE_CONTENT]
            if len(self.text) > 0
            else []
        )
        for subsection in self.subsections:
    ...
    def hierarchy_string(self, tab_counter=0):
        ...
        tab_counter += 1
        hierarchy = [
            TAB * tab_counter + self.name + "(" + str(len(self.text.split(" "))) + ")"
        ]
        for subsection in self.subsections:
    ...
    def to_json(self):
        serialized_object = {"name": self.name, "subsections": [], "text": self.text}
...
class ArxivDocument(Section):
    def __init__(self, name=None, id_val=None, title=None, url=None, published=None):
