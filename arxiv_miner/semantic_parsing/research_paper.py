...
class ResearchPaper(object):
    ...
    def from_json(cls, json_object):
        ...
        paper.unknown_sections = [
            Section.from_json(sec_obj) for sec_obj in json_object["unknown_sections"]
        ]
        paper.build()
        return paper
    ...
    def parsing_results(self):
        num_active_sections = sum(
            list(map(lambda x: 1 if x.matched else 0, self.search_sections))
        )
        results = {
            "active": True
            if (num_active_sections > 0 or len(self.unknown_sections) > 0)
            else False,
            "section_matches": [],
