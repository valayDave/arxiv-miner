...
class ScrapingEngine:
    ...
    def _scrape(self, start_date, end_date):
        try:
            scraper = Scraper(
                category=self.selected_class, date_from=start_date, date_until=end_date
            )
        ...
        self.logger.info(
            "Scraped %d Records From %s To %s"
            % (len(scraped_id_set), start_date, end_date)
        )
    ...
    def _save_oa2_identity(self, oa2_record):
        ...
        if (
            db_retrieved_identity
        ):  # If we can find the Doc in DB then continue as there is no more need to check/Add
        ...
        self.db.set_semantic_parsed_research(
            ArxivSematicParsedResearch(
                ...
                research_object=ResearchPaperFactory.from_arxiv_record(
                    ArxivRecord(identity=retrieved_identity)
                ),
            )
        )
        return True
...
class DailyScrapingEngine(ScrapingEngine):
    ...
    def __call__(self):
        ...
        try:
            ...
            self.logger.error(
                "Failure At Scraping For Dates %s To %s" % (today, yesterday)
            )
        ...
        self.logger.info(
            "Saved %d Records For %s To %s" % (new_added, yesterday, today)
        )
...
class MassDataHarvestingEngine(ScrapingEngine):
    ...
    def __init__(
        ...
        database,
        selected_class="cs",
        start_date=DEFAULT_SCRAPE_UNTIL,
    ...
    def get_date_arr(self, start_date: datetime.datetime, end_date: datetime.datetime):
        ...
        for i in range(delta.days + 1):
            ...
            date_arr.append(
                (
                    save_st_day.strftime(self.date_format),
                    save_end_day.strftime(self.date_format),
                )
            )
    ...
    def __call__(self):
        for start_date, end_date in self.scraping_dates:
            ...
            try:
                ...
                time.sleep(self.timeout_per_scrape)
                self.logger.error(
                    "Failure At Scraping For Dates %s To %s" % (start_date, end_date)
                )
                continue
            for cache_missed_paper in db_check_records:
                scrape_status.append(self._save_oa2_identity(cache_missed_paper))
            new_added = sum([1 for st in scrape_status if st])
            self.logger.info(
                "Saved %d Records For %s To %s" % (new_added, start_date, end_date)
            )
            if len(self.scraping_dates) > 1:
                time.sleep(
                    self.timeout_per_scrape
                )  # Sleep For X Number of Seconds Before One Starts Again.
...
class HarvestingThread(Thread):
    ...
    def shutdown(self, signal_received, frame):
        ...
        self.scraping_engine.logger.info(
...
class DailyHarvestationThread(HarvestingThread):
    ...
    def run(self):
        while True:
            ...
            self.scraping_engine.logger.info(
                "Sleeping For %d Seconds Post Scraping " % self.timeout_per_scrape
            )
            time.sleep(self.timeout_per_scrape)
