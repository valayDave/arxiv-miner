...
class MiningEngine:
    ...
    def __init__(self, database: ArxivDatabase, data_root_path, detex_path):
    ...
    def mine_record(self, paper_record: ArxivRecord):
        paper_obj = ArxivPaper.from_arxiv_record(
            self.data_root_path, paper_record, detex_path=self.detex_path
        )
        try:
            paper_obj.mine_paper()
        except Exception as e:
            self.logger.error(
                "Failed Mining Paper : %s\n\n%s" % (paper_obj.identity.identity, str(e))
            )
            return None
    ...
    def _paper_mining_logic(self):
        ...
        try:
            self.db.set_many_authors(
                [Author(name=xp) for xp in paper_record.identity.authors]
            )
        except:
    ...
    def harvest(self):
        ...
        while len(self.id_list) > 0:
            ...
            try:
                ...
                self.logger.error(
                    f"Latex Download {str(e)} For ID {arxiv_id}. Will be Sleeping for {self.error_sleep_time}"
                )
    ...
    def start_mining(self):
        while True:
            ...
            if not paper_record:  # Sleep If DB says There are Unmined Papers
                self.logger.info(
                    "No Record Found Sleeping For %d" % self.empty_wait_time
                )
                time.sleep(self.empty_wait_time)
                continue
            ...
            if mined_status is False:
                self.logger.error(
                    "Couldnt Mine Paper  : %s" % paper_record.identity.identity
                )
                time.sleep(self.empty_wait_time)
                continue
            self.logger.info(
                "Saved Paper To DB : %s Completed Mining %d Paper"
                % (paper_record.identity.identity, self.num_mined)
            )
