...
from .semantic_parsing import ArxivDocument, ResearchPaper
...
class ArxivIdentity:
    ...
    def from_oa2_response(cls, oa2Record):
        ...
        identity_dict = {
            ...
            "url": oa2Record[
                "url"
            ],  # Version changes come here. Should they be pruned ? TODO
            ...
            "updated": oa2Record["created"]
            if oa2Record["updated"] == ""
            else oa2Record["created"],
    ...
    def from_arxiv_response(cls, arxiv_response):
        ...
        identity_dict = {
            ...
            "updated": arxiv_response["published"]
            if arxiv_response["updated"] == ""
            else arxiv_response["updated"],
...
class ArxivPaperProcessingMeta:
    ...
    def __init__(
        ...
        self.updated_on = dateparser.parse(
            str(updated_on)
        )  # .isoformat() # When was it Mined.
...
class ArxivRecord(object):
    ...
    def to_json(self):
        data_dict = {
            ...
            "latex_parsed_document": None
            if self.latex_parsed_document is None
            else self.latex_parsed_document.to_json(),
    ...
    def from_json(cls, json_object):
        ...
        if (
            "paper_processing_meta" in json_object
            and json_object["paper_processing_meta"]
        ):
            paper_processing_meta = ArxivPaperProcessingMeta(
                **json_object["paper_processing_meta"]
            )
        if (
            "latex_parsing_result" in json_object
            and json_object["latex_parsing_result"]
        ):
            latex_parsing_result = ArxivLatexParsingResult(
                **json_object["latex_parsing_result"]
            )
        if (
            "latex_parsed_document" in json_object
            and json_object["latex_parsed_document"]
        ):
            latex_parsed_document = ArxivDocument.from_json(
                json_object["latex_parsed_document"]
            )
...
class ArxivPaperStatus:
    ...
    def __init__(
        self, mined=False, created_on=None, scraped=False, mining=False, updated_on=None
    ):
