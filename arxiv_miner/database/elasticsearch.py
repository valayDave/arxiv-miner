...
class ArxivElasticSeachDatabaseClient(ArxivDatabase):
    def __init__(self, index_name=None, host="localhost", port=9200, auth=None):
        ...
        if auth is None:
            ...
            self.es = elasticsearch.Elasticsearch(
                src_str, http_auth=auth, timeout=30, max_retries=10
            )
        if not self.es.ping():
            if port is None:
    ...
    def _save_status(self, status: ArxivPaperStatus, identity: ArxivIdentity):
        ...
        self.es.index(
            index=self.status_index_name, id=identity.identity, body=status_doc
        )

    def _save_record(self, record: ArxivRecord):
        record_doc = record.to_json()
        self.es.index(
            index=self.index_name, id=record.identity.identity, body=record_doc
        )
    ...
    def get_unmined_paper(self) -> ArxivRecord:
        ...
        query = (
            search.query(~Q("exists", field="paper_processing_meta.mined"))
        ...
        s = (
            Search(using=self.es, index=self.status_index_name)
            .query(query)
            .filter(id_filter)
        )
        ...
        status_dict = (
            hit_choice.to_dict()
        )  # elasticsearch-dsl directly returns the doc.
        ...
        )  # Update DB Document to avoid reextraction
        return record
    ...
    def record_stream(self) -> ArxivRecord:
        ...
        :yield: [ArxivRecord]
        """
        search_obj = Search(using=self.es, index=self.index_name).query(Q())
    ...
    def get_id_list(self, id_list, page_number=1, page_size=10, with_total=False):
        search_obj = Search(using=self.es, index=self.parsed_research_index_name).query(
            Q("ids", **dict(values=id_list))
        )
        if page_size > 0:
        ...
        if not with_total:
            return [
                ArxivSematicParsedResearch.from_json(hit.to_dict()) for hit in text_res
            ]
        else:
            return (
                text_res.hits.total.value,
                [
                    ArxivSematicParsedResearch.from_json(hit.to_dict())
                    for hit in text_res
                ],
            )
    ...
    def archive(self):
        query = Q()
        s = Search(using=self.es, index=self.status_index_name).query(Q())
...
SOURCE_FIELDS = ["identity.*", "research_object.parsing_stats"]
...
class TextSearchFilter:
    ...
    def __init__(
        self,  # Ids that act as addition filter layer
        id_vals=[],  # no_date : bool for not using date filter
        no_date=False,  # Text filter opt
        string_match_query="",
        text_filter_fields=[],
        # Date filter opt
        start_date_key=None,
        end_date_key=None,
        date_filter_field=DATE_FIELD_NAME,  # Category filter :CategoryFilterItem  : use multi_category_filter or category_filter or category_filter_values,category_field,category_match_type
        category_filter=[],
        ...
        sort_order="descending",  # highlight opts : Aggregation sets this as [] default.
        highlights=TEXT_HIGHLIGHT,
        highlight_fragments=60,  # Source fields,
        source_fields=[],  # Page settings
        page_size=10,
        page_number=1,
        scan=False,  # Full Dataset Traversal key # If scan==True, then no Pagination else paaginate
    ):

        self.text_search_query = self._text_search_query(
            string_match_query, text_filter_fields
        )
        self.date_filter = self._date_query(
            start_date_key, end_date_key, date_range_key=date_filter_field
        )
        if multi_category_filter and len(multi_category_filter) > 0:
            ...
            self.category_filter = self._subcategory_filter(
                category_filter_values, category_field, match_type=category_match_type
            )
    ...
    def _multicategory_filter(
        self, category_filter_items: List[List[CategoryFilterItem]]
    ):
        combined_query = None
    ...
    def _date_query(start_date_key, end_date_key, date_range_key="identity.published"):
        default_start_date, default_end_date = get_date_range_from_today(
            DEFAULT_TIME_RANGE
        )
        ...
        unroll_json = dict()
        unroll_json[date_range_key] = {"gte": start_date_key, "lte": end_date_key}
    ...
    def _text_search_query(string_match_query, text_filter_fields):
        ...
        es_builder = ElasticsearchQueryBuilder(
            default_field="*", field_options=field_options
        )
    ...
    def query(self):
        ...
        if not self.no_date:
            final_filter.append(Q("range", **self.date_filter))
        # Text Query setting
        if self.text_search_query is not None:
            query_type = list(self.text_search_query.keys())[0]
            final_filter.append(Q(query_type, **self.text_search_query[query_type]))

        # Category Query setting
        if self.category_filter is not None:
            query_type = list(self.category_filter.keys())[0]
            final_filter.append(Q(query_type, **self.category_filter[query_type]))
        ...
        for highlight in self.highlights:
            search_obj = search_obj.highlight(
                highlight, fragment_size=self.highlight_fragments
            )
...
class Aggregation(TextSearchFilter):
    ...
    def __init__(self, no_docs=True, agg_name="", **kwargs):
    ...
    def transform_resp(doc_list: List[dict], metric_name="doc_count"):
        arr = []
        for doc in doc_list:
            arr.append(dict(key=doc["key"], metric=doc[metric_name]))
        return arr
...
class DateAggregation(Aggregation):
    ...
    def transform_resp(doc_list: List[dict], metric_name="doc_count"):
        ...
        for doc in doc_list:
            arr.append(
                dict(
                    key=dateparser.parse(doc["key_as_string"]), metric=doc[metric_name]
                )
            )
        return arr
...
class TermsAggregation(Aggregation):
    def __init__(
        self, field=CATEGORY_FIELD_NAME, agg_name="terms_aggregation", **kwargs
    ):
        super().__init__(agg_name=agg_name, **kwargs)
        self.field = field
...
class ArxivElasticTextSearch(ArxivElasticSeachDatabaseClient):
    ...
    def text_search_scan(self, filter_obj: TextSearchFilter):
        ...
        search_generator = (
            Search.from_dict(search_query).using(self.es).index(self.index_name)
        )
    ...
    def text_search(self, filter_obj: TextSearchFilter) -> List[SearchResults]:
        ...
        text_res = (
            Search.from_dict(filter_obj.query())
            .using(self.es)
            .index(self.parsed_research_index_name)
            .execute()
        )
        ...
        for hit in text_res:
            ...
            if "highlight" in meta_dict:
                ...
                for key in list(meta_dict["highlight"].keys()):
                    ...
                    frag_obj["highlight"] = self.fragment_from_hightlight(
                        meta_dict["highlight"][key]
                    )
    ...
    def fragment_from_hightlight(hightlights: List[str], tag="em"):
        ...
        for frag in hightlights:
            ...
            new_word = []
            highlight_frag = dict(
                words=re.findall(reg_str, frag), text=" ".join(re.split(reg_str, frag))
            )
    ...
    def text_aggregation(self, agg_obj: Aggregation):
        ...
        aggregation_buckets = text_agg_res.aggregations.to_dict()[agg_obj.agg_name][
            "buckets"
        ]
...
class KeywordsTextSearch(ArxivElasticTextSearch):
    ...
    def set_author(self, author_obj: Author):
        ...
        self.es.index(self.authors_index, author_dict, id=author_obj.name)
    ...
    def set_ontology(self, ontlist: List[str]):
        for ont_str in ontlist:
            ...
            ont_dict = D2D(ont)
            self.es.index(self.ontology_index, ont_dict, id=ont.value)
    ...
    def autocomplete_authors(self, fragment: str, max_frags=10) -> List[str]:
        sugget_dict = dict(
            suggest=dict(
                authors_completion=dict(
                    prefix=fragment,
                    completion=dict(
                        field=self.authors_autocomplete_field_name, size=max_frags
                    ),
                )
            )
        )
        ...
        if len(search_results["suggest"]["authors_completion"]) > 0:
            for suggest in search_results["suggest"]["authors_completion"][0][
                "options"
            ]:
                txt = suggest["text"]
    ...
    def autocomplete_ontology(self, fragment: str, max_frags=10) -> List[str]:
        sugget_dict = dict(
            suggest=dict(
                ontology_completion=dict(
                    prefix=fragment,
                    completion=dict(
                        field=self.ontology_autocomplete_field_name, size=max_frags
                    ),
                )
            )
        )
        ...
        if len(search_results["suggest"]["ontology_completion"]) > 0:
            for suggest in search_results["suggest"]["ontology_completion"][0][
                "options"
            ]:
                txt = suggest["text"]
                return_text.append(txt)
